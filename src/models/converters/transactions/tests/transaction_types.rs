...
use crate::models::service::transactions::{TransactionInfo, Custom, Transfer, EtherTransfer, TransferInfo, TransferDirection, SettingsChange, SettingsInfo, Erc20Transfer, Erc721Transfer};
...
fn transaction_data_decoded_is_erc20_receiver_ok_transfer_method() {
    ...
    let tx = serde_json::from_str::<MultisigTransaction>(crate::json::MULTISIG_TX_ERC20_TRANSFER).unwrap();
    let expected = TransactionInfo::Transfer(Transfer {
        sender: "0x1230B3d59858296A31053C1b8562Ecf89A2f888b".to_string(),
...
fn transaction_data_decoded_is_erc721_receiver_ok_transfer_method() {
    ...
    let tx = serde_json::from_str::<MultisigTransaction>(crate::json::MULTISIG_TX_ERC721_TRANSFER).unwrap();
    let expected = TransactionInfo::Transfer(Transfer {
        sender: "0x1230B3d59858296A31053C1b8562Ecf89A2f888b".to_string(),
        recipient: "0x938bae50a210b80EA233112800Cd5Bc2e7644300".to_string(),
        direction: TransferDirection::Outgoing,
...
fn transaction_data_decoded_is_erc20_receiver_not_ok_transfer_method() {
    ...
    mock_info_provider
        .expect_token_info()
        .times(0);
    let tx = serde_json::from_str::<MultisigTransaction>(crate::json::MULTISIG_TX_ERC20_TRANSFER_INVALID_TO_AND_FROM).unwrap();
    let expected = TransactionInfo::Custom(Custom {
        to: "0xD9BA894E0097f8cC2BBc9D24D308b98e36dc6D02".to_string(),
        data_size: "68".to_string(),
        value: "0".to_string(),
