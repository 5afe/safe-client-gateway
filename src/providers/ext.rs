...
pub trait InfoProviderExt: InfoProvider {
    async fn address_to_token_info(
        ...
        self.token_info(chain_id, address).await.ok()
    }
    ...
    async fn addresses_to_address_ex(
        ...
        for address in addresses {
            results.push(self.to_address_ex(chain_id, address).await)
        }
        Some(results)
    }
    ...
    async fn to_address_info(
        ...
        OptionFuture::from(
            address
                .as_ref()
                .map(|address| async move { self.contract_info(chain_id, address).await.ok() }),
        )
        .await
        .flatten()
    }
    ...
    async fn optional_to_address_ex(
        ...
        OptionFuture::from(
            address
                .as_ref()
                .map(|address| async move { self.to_address_ex(&chain_id, address).await }),
        )
        .await
    }
}
