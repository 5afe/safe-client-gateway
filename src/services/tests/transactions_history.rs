use crate::json::BACKEND_TRANSACTION_LIST_PAGE_01;
...
fn backend_txs_to_summary_txs_with_values() {
    let backend_txs =
        serde_json::from_str::<Page<Transaction>>(BACKEND_TRANSACTION_LIST_PAGE_01).unwrap();
    let mut mock_info_provider = MockInfoProvider::new();
    mock_info_provider.expect_safe_info().times(0);
    mock_info_provider.expect_token_info().times(0);
    let mut back_end_txs_iter = backend_txs.results.into_iter();
...
fn peek_timestamp_and_remove_item_with_items() {
    ...
    mock_info_provider.expect_safe_info().times(0);
    mock_info_provider.expect_token_info().times(0);
    let backend_txs = serde_json::from_str::<Page<Transaction>>(BACKEND_TRANSACTION_LIST_PAGE_01)
        .unwrap()
        .results;
    let mut backend_txs_iter = backend_txs.into_iter();
